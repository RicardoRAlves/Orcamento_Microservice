...
public class BudgetCalculationResultConsumer {

    private static final Logger LOGGER = LoggerFactory.getLogger(BudgetCalculationResultConsumer.class);
    ...
    public BudgetCalculationResultConsumer(
            SqsClient sqsClient,
    ...
    public void initializeQueueUrl() {
        ...
        LOGGER.info("Budget calculation result consumer connected to queue. queueName={}, queueUrl={}", queueName, queueUrl);
    }
    ...
    public void poll() {
        try {
            ...
            LOGGER.error("Could not poll budget calculation result queue. queueName={}, error={}", queueName, exception.getMessage(), exception);
        }
    }

    private void handleMessage(Message sqsMessage) {
        try {
            var calculationResult = objectMapper.readValue(sqsMessage.body(), BudgetCalculationResultMessage.class);
            budgetOrchestrationService.finishCalculation(calculationResult);
            deleteMessage(sqsMessage);
        } catch (JsonProcessingException exception) {
            LOGGER.error(
                    "Invalid budget calculation result message. It will not be deleted and should be moved to DLQ after retries. messageId={}, body={}",
            ...
            LOGGER.error(
                    "Could not process budget calculation result message. It will not be deleted and should be retried by SQS. messageId={}, error={}",
                    sqsMessage.messageId(),
